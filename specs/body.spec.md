...
own BODY-15/BODY-16. A refusal reaching the shared collection point prints
...
- [ ] [BDY-08] `aco body --check` prints every defect sentence on stderr, one per line, never truncated like `aco check` (CHECK-06, CHECK-07); exit `1`.
- [ ] [BDY-09] `aco body --check --json` prints `{"ok": <bool>, "defects": [...]}`, the same ordered sentences BDY-08 lists in text; exit `0` or `1` to match (see E-BDY-03).
