...
`<claim-id>` is the released claim's own id. `<subject>` is `_claim_subject`'s
grammar, unquoted: `issue <label>` or `lane <branch>` (`<label>` is
...
- [ ] [REL-02] An `--abandoned` value that is empty, padded, carries a control character, spans more than one line, or exceeds 512 characters refuses `abandoned reason must be one bounded non-empty line`, exit `2`.

## Identity and branch resolution

Omitting the positional issue number and an explicit `--branch` both select
lane mode from `_resolved_identity`, the same resolver `aco claim`'s own
lane mode uses; a future `claim` spec would cite REL-03 rather than restate
it.
...
- [ ] [REL-06] `--branch` omitted, no issue number, and an empty current branch refuses `lane release requires a non-empty current branch; check out the docs/ or fix/ lane branch, or pass an issue number`, exit `2`.
- [ ] [REL-07] `--branch` omitted, an issue number given without `--claim-id`, and an empty current branch refuses `release without --claim-id requires a non-empty current branch; pass --claim-id`, exit `2`.
...
- [ ] [REL-11] `--branch`/`--claim-id` disagreement refuses `--branch '<branch>' and --claim-id '<claim-id>' disagree: the claim's own branch is '<branch>'; drop --branch or pass its own value`, exit `2` (E-REL-04).
- [ ] [REL-12] A release by the wrong agent/role, no coordinator override, refuses `only the original claimant may release` (CLAIM-38), before any write.
...
- [ ] [REL-14] Omitting `--role` leaves the parsed value `None` -- unlike `claim`, whose omitted `--role` defaults to `builder` -- and the release reports the claim's own stored role.
...
- [ ] [REL-16] `release --merged <pr>`'s own forge verification, run before any store write, is `specs/landing-grammar.spec.md`'s (LAND-29..LAND-40, LAND-49, LAND-50); this file adds nothing to that grammar.
- [ ] [REL-17] `release --merged` under `storage = "state-ref"` refuses before the forge is resolved (LAND-40, PIN-12's shared sentence; see `specs/storage-pin.spec.md` E-PIN-06).
...
- [ ] [REL-18] A successful release, either outcome, without `--json` prints exactly `RELEASED <subject>: <claim-id>`, one line (see E-REL-01, E-REL-02).
...
- [ ] [REL-20] A resolved `--merged` landing adds `freed:`/`next:` after `RELEASED` in text, or `freed`/`next` keys to `--json`, present only then (see E-REL-02).
...
- [ ] [REL-22] A `--merged` release hitting an unreachable forge post-commit prints `hint:` (LAND-38) on stdout in text, or on stderr beside `--json`; `freed`/`next` are omitted, the release itself stands (LAND-50).
