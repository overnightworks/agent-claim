...
The tool keeps every claim in one GitHub issue whose comments are the ledger.
...
5. **One state-ref cut** — claim, rescope, both release outcomes, coordinator override, stale
   ...
   to seed from. The import ran on 07.09.2026 and its code was deleted afterwards (issue #187),
   so `bootstrap` today only creates or reports the ref.
...
05.09.2026 against `overnightworks/agent-coordination` and neither counter-check could reproduce it
...
|---|---|---|
| GitHub accepts pushes to `refs/aco/*`; create-if-absent, fast-forward update, non-fast-forward rejection, concurrent rejection (two parallel pushes of different children from two clones → exactly one winner, the other "cannot lock ref"), a stale-lease create rejected, and delete all behave as required; the REST endpoint `git/refs/agent-coordination` returns 200 while the ref is present and 404 after deletion; the ref is invisible in the normal GitHub UI | Verified 05.09.2026 (re-run by the head, results on #131) | Head's live re-run, 05.09.2026, `overnightworks/agent-coordination`, namespace `refs/agent-coordination/probe-1788623638` (deleted afterwards) |
...
| GitHub rulesets and branch protection target branches and tags only, never `refs/aco/*` | Verified (documentation) | GitHub docs. Accepted: the threat model is cooperating clients |
| GitHub supports organization-owned custom issue types | Verified (documentation) | GitHub issue-type API |
| `refs/aco/*` is pushable on GitLab | **Unverified** — the Branches API proves only `refs/heads` creation, and Gitaly's note about other namespaces certifies neither GitLab.com nor every installation | Round-2 counter-check |
...
| 0.13.0's typed body block (D6) was green in the owner repository — passed final gate, four counter-check rounds — before atelier-2 pinned it in PR #1377 (06.09.2026 18:37 UTC). Two regressions followed: `next` printed a cut title `cut` refused on seven of twenty-one containers, found in that PR's required review 18:38–19:05 UTC; and that PR's CI job "Landing classification" — which runs `pr-check` without a checkout — failed with git's raw error (`fatal: not a git repository`, exit 2) at the pin, because every contract-resolving command now resolves its config from the working tree; fixed on their side with a checkout step, green again at 18:54:35 UTC | Owner side (green, final gate, four counter-check rounds) measured in this repository; consumer side (both findings and their timing) measured, atelier-2's own account | Owner side: PR #162, PR #165, #150's counter-check reports. Consumer side: atelier-2's record of PR #1377 |
| No rule followed from that finding: the divergent cut-title shape already sat on this repository's own board, hidden behind a child that was still open; the round-trip test suite's 2×2 matrix over {`next`-line present} × {uncut `[[slice]]` row present} left empty exactly the cell where both are present and disagree; the autouse toplevel-resolution test fixture made a missing working tree unobservable in every test — the blindness was local and testable, not structural. What owns a fix instead of a rule: that empty matrix cell and the next/cut title invariant over every container (#177); the fixture's counterpart (#178); per-command README preconditions (step-5 plan, slice F); and keeping "which consumer shape went unexercised" a dispatch-brief practice, not a tracked item | Measured; proposed rule rejected by independent counter-check | Independent counter-check, 06.09.2026; #177; #178 |
...
| D7 | The package renames to `agent-coordination` and the command to `aco`, one name each with no alias kept for the old ones. Ruled 06.09.2026; timing amended 07.09.2026 (H1, issue #176): the rename lands in slice F, after the state-ref cut (step 5) and its one-time ledger import, not in the same release as the cut — the import checklist still runs `bootstrap --ledger N` under the old console script of a 1.0-but-not-yet-renamed build, since renaming the console script the import itself depends on would break the checklist mid-migration. That import ran on 07.09.2026 and `--ledger` was deleted with the rest of the import code (issue #187), so slice F now renames a `bootstrap` that only creates the state ref. Implemented 07.09.2026 (issue #191): the distribution is `agent-coordination` and its one console script is `aco` | Command, package, and storage model already break together at the rename release, so carrying an alias forward would promise a continuity that release does not otherwise offer; H1 separates "the storage model breaks" (step 5) from "the name breaks" (slice F) so the import has one fixed tool name to run under, not a moving target |
