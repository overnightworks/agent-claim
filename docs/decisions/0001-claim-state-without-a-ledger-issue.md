...
| No rule followed from that finding: the divergent cut-title shape already sat on agent-claim's own board, hidden behind #122's open child #176; the round-trip test suite's 2×2 matrix over {`next`-line present} × {uncut `[[slice]]` row present} left empty exactly the cell where both are present and disagree; the autouse toplevel-resolution test fixture made a missing working tree unobservable in every test — the blindness was local and testable, not structural. What owns a fix instead of a rule: completing that matrix cell and checking the `next`/`cut` title invariant over every container, not only the top-ranked one, in #177 (in build); the fixture's counterpart in #178 (open); per-command preconditions in the README in slice F of the step-5 plan; and keeping "which consumer shape went unexercised" a dispatch-brief practice, not a tracked item | Measured; proposed rule rejected by independent counter-check | Independent counter-check, 06.09.2026; #177; #178 |
