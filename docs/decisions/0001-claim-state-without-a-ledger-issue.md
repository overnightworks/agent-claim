...
| 0.13.0's typed body block (D6) was green in the owner repository — passed final gate, four counter-check rounds — then, pinned in the first consuming repository's PR #1377 (06.09.2026 18:37 UTC), found two regressions within the half hour that followed: `next` printing a cut title `cut` refused on seven of twenty-one containers, and every contract-resolving command newly requiring a working tree, refused with git's raw error | Measured, atelier-2's own account | atelier-2's record of PR #1377, 06.09.2026 18:37–19:05 UTC |
| No rule followed from that finding: the divergent cut-title shape already sat on agent-claim's own board, hidden behind an open child; the round-trip test matrix for the `next`/`cut` invariant had exactly that disagreeing cell empty; the autouse toplevel-resolution test fixture made a missing working tree unobservable in every test — the blindness was local and testable, not structural. What replaced it: the matrix completed, every container checked instead of only the top-ranked one, the fixture given its counterpart, per-command preconditions enumerated in the README, and "which consumer shape went unexercised" kept as a dispatch-brief question, not a gate | Measured; proposed rule rejected by independent counter-check | Independent counter-check, 06.09.2026 |
