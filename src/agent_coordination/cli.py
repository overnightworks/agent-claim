...
def _body_template(kind: str, parent: int | None) -> str:
    ...
    same `body.BLOCK_CHILD_SKELETON` `cut` writes for a task or feature
    child, or `body.BLOCK_CONTAINER_SKELETON` for a container -- with
    `parent`'s own `Parent:` line ahead of it when given, exactly as `cut`
    composes one for its own fresh child (`_body_with_parent`)."""
    skeleton = body.BLOCK_CONTAINER_SKELETON if kind == "container" else body.BLOCK_CHILD_SKELETON
